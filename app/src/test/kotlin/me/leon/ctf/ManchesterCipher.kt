...
fun ByteArray.manchester(isStandard: Boolean = false) =
    toBinaryString().map {
        if (isStandard) ENCODE_STANDARD[(it - '0')]
        else ENCODE_8023[(it - '0')]
    }.joinToString("")

fun String.manchesterDecode(isStandard: Boolean = false) =
    chunked(2).map {
        if (isStandard) ENCODE_STANDARD.indexOf(it)
        else ENCODE_8023.indexOf(it)
    }.joinToString("")

fun ByteArray.manchesterDiff() =
    toBinaryString().foldIndexed(StringBuilder()) { index, acc, c ->
        acc.apply {
...
fun String.manchesterDiffDecode() =
    chunked(2).map { ENCODE_STANDARD.indexOf(it) }.joinToString("")
