 ...
 *
 * <p>A standard {@code PasswordEncoder} implementation that uses SHA-256 hashing with 1024
 * iterations and a random 8-byte random salt value. It uses an additional system-wide secret value
 * to provide additional protection.
 *
 * <p>The digest algorithm is invoked on the concatenated bytes of the salt, secret and password.
 *
 * <p>If you are developing a new system, {@link bcrypt.BCryptPasswordEncoder} is a better choice
 * both in terms of security and interoperability with other languages.
 *
 * @author Keith Donald
 * @author Luke Taylor
    ...
    private byte[] digest(CharSequence rawPassword, byte[] salt) {
        byte[] digest =
                this.digester.digest(
                        EncodingUtils.concatenate(salt, this.secret, Utf8.encode(rawPassword)));
