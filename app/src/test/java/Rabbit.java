...
public class Rabbit {
    ...
    /**
     * A constants due to <a href=https://tools.ietf.org/html/rfc4503#section-2.5">Counter
     * system</a>
     */
    ...
    /**
     * Encrypt given message with given charset, using Key to crypt and given IV. If addPadding is
     * set - string will be padded with zeros to be multiple of KEYSTREAM_LENGTH
     *
     * @param message message to be encrypted
     * @param charset message charset
     * @param key key
     * @param iv IV
     * @param addPadding padding indicator
     * @return encrypted byte array
     */
    public byte[] encryptMessage(
            final String message, Charset charset, String key, String iv, boolean addPadding) {
        if (message == null
                || key == null
                || charset == null
                || message.isEmpty()
    ...
    /**
     * Encrypt given message with given pre-defined UTF-8 charset, using Key to crypt and given IV.
     * If addPadding is set - string will be padded with zeros to be multiple of KEYSTREAM_LENGTH
     *
     * @param message message to be encrypted message charset
     * @param key key
     * @param iv IV
     * @param addPadding padding indicator
     * @return encrypted byte array
     ...
     */
    public byte[] encryptMessage(final String message, String key, String iv, boolean addPadding) {
        return encryptMessage(message, StandardCharsets.UTF_8, key, iv, addPadding);
    }

    /**
     * Decrypt given byte array to String with given charset, using Key and IV, and if timePadding
     * is set - with omitted leading and trailing whitespace.
     *
     * @param encMessage message byte array to decrypt
     * @param charset charset of needed string decrypted equivalent
     * @param key key
     * @param iv IV
     * @param trimPadding padding indicator
     * @return String with in given Charset encoding
     */
    public String decryptMessage(
            final byte[] encMessage, Charset charset, String key, String iv, boolean trimPadding) {
    ...
    /**
     * Decrypt given byte array to String with given pre-defined UTF-8 charset, using Key and IV,
     ...
     */
    public String decryptMessage(
            final byte[] encMessage, String key, String iv, boolean trimPadding) {
        return decryptMessage(encMessage, StandardCharsets.UTF_8, key, iv, trimPadding);
    }

    /**
     * Add padding to given message byte array
     *
     * @param message byte array to add padding
     * @return padded byte array
     */
    private byte[] addPadding(final byte[] message) {
        if (message.length % KEYSTREAM_LENGTH != 0) {
            return Arrays.copyOf(message, message.length + message.length % KEYSTREAM_LENGTH);
        } else {
            return message;
        }
    }
     ...
     * Crypt function
     *
     * @param message byte array to be crypted. Should be fed an array with a length that is
     *     multiple of 16 for proper key sequencing.
     * @return array of crypted bytes
     * @see <a href="https://tools.ietf.org/html/rfc4503#section-2.8">Encryption/Decryption
     *     Scheme</a>
     */
     ...
     * S-block extraction
     *
     * @see <a href="https://tools.ietf.org/html/rfc4503#section-2.7">Extraction Scheme</a>
     */
     ...
     * @see <a href="https://tools.ietf.org/html/rfc4503#section-2.6">Next-State Function
     *     RFC4503</a>
     */
     ...
     * @see <a href="https://tools.ietf.org/html/rfc4503#section-2.4">Setup IV</a>
     */
     ...
     * @see <a href="https://tools.ietf.org/html/rfc4503#section-2.3">Setup key</a>
     */
