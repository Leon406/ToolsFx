...
const val UTF7_Base64 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz01234567890/,"
...
fun String.utf7(isAll: Boolean = false) =
    if (isAll) "+${toByteArray(Charsets.UTF_16BE).base64(UTF7_Base64, false)}-"
    else {
        StringBuilder(this).replace(SET_ALL) {
            val base64 =
                if (it.value == "+") ""
                else it.value.toByteArray(Charsets.UTF_16BE).base64(UTF7_Base64, false)
            "+$base64-"
        }
    }
...
fun String.uft7Decode() =
    StringBuilder(this).replace(DECODE) {
        ...
        if (s == "") "+" else s.base64Decode(UTF7_Base64).toString(Charsets.UTF_16BE)
    }
