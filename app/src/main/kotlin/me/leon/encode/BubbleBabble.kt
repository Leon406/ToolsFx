...
object BubbleBabble {
    ...
    fun encrypt(str: String): String {
        ...
        for (i in 0 until k / 2 + 1) {
            C[i] =
                if (i == 0) 1 else ((C[i - 1] * 5 + D[((i + 1) * 2) - 3 - 1] * 7 + D[((i + 1) * 2) - 2 - 1]) % 36).toByte()
        }
    ...
    fun decrypt(str: String): String {
        ...
        return tmpStr.chunked(6).foldIndexed(StringBuilder()) { index, acc, value ->
            acc.apply {
...
fun ByteArray.bubbleBabble(charset: String) = BubbleBabble.encrypt(String(this, Charset.forName(charset)))
