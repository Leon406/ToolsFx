...
fun String.adfgvx(table: String, keyword: String): String {
    val key = keyword.toList().distinct().joinToString("")
    val polybius = polybius(table, ADFGVX_ENCODEMAP, "I" to "I")
    val keyM =
...
fun String.adfgvxDecrypt(table: String, keyword: String): String {
    val key = keyword.toList().distinct().joinToString("")
    ...
    return chunked(key.length)
        ...
        .joinToString("")
        .polybiusDecrypt(table, ADFGVX_ENCODEMAP)
}
