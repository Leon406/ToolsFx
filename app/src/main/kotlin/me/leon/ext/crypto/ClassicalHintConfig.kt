...
val CLASSIC_CONFIG =
    mapOf(
        ClassicalCryptoType.CAESAR to
            mapOf(HINT to arrayOf("shift", "shift lower(default is same to shift)")),
        ClassicalCryptoType.AFFINE to mapOf(HINT to arrayOf("factor a", "b")),
        ClassicalCryptoType.RAILFENCE to
            mapOf(
                HINT to arrayOf("fence number", "offset,default is 0 (w-type)"),
                CHECK to arrayOf("W-type"),
            ),
        ClassicalCryptoType.VIRGENENE to mapOf(HINT to arrayOf("key")),
        ClassicalCryptoType.MORSE to mapOf(HINT to arrayOf("default .", "default -")),
        ClassicalCryptoType.POLYBIUS to
            mapOf(
                HINT to arrayOf("table, $TABLE_A_Z_WO_J as default", "encode map, 12345 as default")
            ),
        ClassicalCryptoType.NIHILIST to
            mapOf(HINT to arrayOf("keyword", "encodeMap 12345 is as default")),
        ClassicalCryptoType.ADFGX to
            mapOf(HINT to arrayOf("table $TABLE_A_Z_WO_J", "keyword"), CHECK to arrayOf("ADFGVX")),
        ClassicalCryptoType.PLAYFAIR to mapOf(HINT to arrayOf("key")),
        ClassicalCryptoType.AUTOKEY to mapOf(HINT to arrayOf("key")),
        ClassicalCryptoType.OTP to mapOf(HINT to arrayOf("key data as long as data size")),
        ClassicalCryptoType.AlphabetIndex to
            mapOf(HINT to arrayOf("table, '$TABLE_A_Z' as default", "delimiter(space as default)")),
        ...
        ClassicalCryptoType.CurveCipher to mapOf(HINT to arrayOf("row", "column")),
        ClassicalCryptoType.EmojiSubstitute to mapOf(HINT to arrayOf("shift, default 0")),
        ClassicalCryptoType.HandyCode to
            mapOf(HINT to arrayOf("default table '$TABLE_HANDY_CODE'")),
        ClassicalCryptoType.Porta to mapOf(HINT to arrayOf("key,PORTA as default")),
        ClassicalCryptoType.Beaufort to mapOf(HINT to arrayOf("key,beaufort as default")),
        ClassicalCryptoType.FourSquare to
            mapOf(HINT to arrayOf("key1, length 25 ", "key2, length 25")),
        ClassicalCryptoType.Gronsfeld to
            mapOf(HINT to arrayOf("key, a sequence of numbers 0-9,default 123456 ")),
        ClassicalCryptoType.Trifid to mapOf(HINT to arrayOf("key,length 27 ", "period, default 5")),
        ClassicalCryptoType.Bifid to
            mapOf(HINT to arrayOf(" key,length 25,default A-Z(w/o J) ", "period, default 5")),
        ...
        ClassicalCryptoType.HILL to
            mapOf(
                HINT to
                    arrayOf("key matrix,like 1 2 0 1 or bcab", "A = 0 as default,if has value A =1")
            ),
        ClassicalCryptoType.Rabbit to mapOf(HINT to arrayOf("password,default is empty string")),
