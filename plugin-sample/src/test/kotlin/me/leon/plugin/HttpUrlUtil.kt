...
//        Proxy(Proxy.Type.DIRECT,InetSocketAddress(host,port))
        ...
        val conn =
            URL(url.takeIf { params.isEmpty() } ?: "$url?${params.toParams()}").openConnection() as HttpURLConnection
        var rsp = ""
        val time = measureTimeMillis {
            conn.requestMethod = "GET"
            makeHeaders(headers).forEach { (t, u) ->
                conn.setRequestProperty(t, u.toString())
            }
            conn.doOutput = true
...
//        Proxy(Proxy.Type.DIRECT,InetSocketAddress(host,port))
        ...
        val time = measureTimeMillis {
            ...
            rsp = if (conn.responseCode == HttpURLConnection.HTTP_OK) {
    ...
    private fun showResponseInfo(url: String, conn: HttpURLConnection, time: Long, rsp: String) {
        ...
        sb.append("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
            .appendLine()
            .append(conn.requestMethod).append(" ")
            .append(url).append("  ${time}ms")
            .appendLine()
            .appendLine()
            .append(conn.headerFields[null]?.joinToString("")).appendLine()
    ...
    private fun showRequestInfo(url: String, conn: HttpURLConnection, req: String) {
        ...
        sb.append(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
            .appendLine()
            .append(conn.requestMethod).append(" ")
            .append(url).append(" HTTP/1.1").appendLine()
...
//            ), mutableMapOf(), true
//        )
        post("https://lab.magiconch.com/api/nbnhhsh/guess", mutableMapOf("text" to "cylx"), isJson = true)
    }

    fun Map<String, Any>.toParams() = entries.joinToString("&") {
        it.key + "=" + it.value
    }
    ...
    fun jsonParse() {
        JsonBuilder()
            ...
            .add("4", 3)
            .build().toString().also { println(it) }
    }

    fun Map<String, Any>.toJson(): String =
        entries.fold(JsonBuilder()) { acc, entry ->
            acc.apply {
                when (entry.value) {
